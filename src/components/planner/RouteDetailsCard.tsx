  ...
  return (
    <div className="p-6 bg-slate-800 border border-slate-700 rounded-xl flex flex-col h-full">
      ...
      {rota && rota.distanciaTotal > 0 && (
        <div className="p-4 mb-6 bg-slate-700/50 rounded-lg flex-shrink-0">
          <div className="flex justify-around items-center text-center">
            <div>
              <p className="text-xs uppercase font-semibold text-slate-400 whitespace-nowrap">
                Distância Total
              </p>
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-sky-400">
                {formatarDuracao(rota.duracaoTotal)}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="overflow-y-auto pr-2 -mr-2 flex-grow">
        ...
        {!isLoading && destinosCount < 2 && (
          <p className="text-slate-500">
            Adicione pelo menos dois destinos para visualizar os detalhes da
            rota.
          </p>
        )}
        ...
        {rota && rota.trechos.length > 0 && !isLoading && (
          <div className="space-y-3">
            {rota.trechos.map((trecho, index) => (
              <div
                key={index}
                className="p-3 border-b border-slate-700 text-sm"
              >
                <div className="flex items-center gap-2 flex-wrap font-semibold text-white">
                  <span className="truncate" title={trecho.origem}>
                    {trecho.origem.split(",")[0]}
                  </span>
                  <span className="text-sky-400">→</span>
                  <span className="truncate" title={trecho.destino}>
                    {trecho.destino.split(",")[0]}
                  </span>
                </div>
                  ...
                  {trecho.tipo === "CARRO" ? (
                    <div className="flex items-center gap-4">
                      <span className="flex items-center gap-2">
                        <FaRoute /> {trecho.distancia}
                      </span>
                      <span className="flex items-center gap-2">
                        <FaRegClock /> {trecho.duracao}
                      </span>
                    </div>
                  ) : (
                    <span className="text-sky-400 font-semibold">
                      ✈️ Rota não calculável (viagem longa ou aérea)
                    </span>
                  )}
                </div>
              </div>
