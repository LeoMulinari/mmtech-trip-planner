...
export function SortableListItem({ destino, onDelete, onEdit, isSubmitting }: SortableListItemProps) {
    // Hook do Dnd-Kit que fornece todas as props necessárias para tornar o item arrastável.
    const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: destino._id! });
    
    // Estilos que aplicam a animação de movimento durante o drag-and-drop.
    const style = { transform: CSS.Transform.toString(transform), transition };

    return (
        <li ref={setNodeRef} style={style} {...attributes}>
            <div className="flex items-center gap-3 p-3 rounded-lg bg-slate-700 w-full group">
                {/* Apenas o ícone recebe os eventos de arrastar., isso libera o resto do item para cliques */}
                ...
                {/* As classes min-w-0 e truncate garantem que o texto não "estoure" o layout e seja cortado com "..." se for muito longo */}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => onDelete(destino._id!)} disabled={isSubmitting} className="p-1 rounded-full text-slate-400 hover:bg-red-500 hover:text-white transition-all">
